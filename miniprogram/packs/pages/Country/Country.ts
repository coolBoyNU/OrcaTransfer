// packs/pages/Country/Country.ts
Page({
  ...
  //列表数据筛选
  duf() {
    let res = this.data.options.map(item => {
      const sub = item.value.substring(0, 1)
      return {
        id: sub,
        value: item.value,
        label: item.label
  ...
  bindKeyInput: function (e: any) {
    ...
    //把中英合并
    let combine = this.data.options.map(item => {
      return {
    ...
    let arr: any = []
    this.data.options.forEach((item, i) => {
      //match() 方法可在字符串内检索指定的值
      let matching = combine[i].value.match(e.detail.value)
      // console.log(matching);
      if (matching != null) {
        //使用中英对比相等就复制item
        if (combine[i].value == matching.input) {
          arr.push(item)
        }
      }
    })
